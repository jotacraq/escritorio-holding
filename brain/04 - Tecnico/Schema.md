...
## Fase 6 (05/09/2026 à noite) — migration 0071, aplicada e provada

0071 `app.regua_boas_vindas` vira `security definer set search_path` (a 0020 corrigiu as irmãs `regua_agendamento`/`regua_pos_sessao` e esqueceu esta — sem isso, escrita direta em `pagamentos` como `service_role` morria com 42501 em `app.enfileirar_mensagem`; achado A3 do mock) + `revoke`/`grant execute` explícito de `enfileirar_mensagem` (só `service_role`) · `registrar_diagnostico_sv(p_jornada_id, p_analise_id, p_blocos, p_criado_por default null)`: com sessão o autor é o perfil da sessão (`p_criado_por` ignorado); sem sessão (`service_role`) `p_criado_por` é obrigatório (22004) e validado por `app.perfil_ve_patrimonio` (42501) — assinatura antiga de 3 args dropada, chamada posicional continua resolvendo. Prova: `scripts/verificacao-0071.sql` 11/11.

**Armadilha do roteiro:** fixture de pagamento aprovado precisa de jornada em etapa com `ordem >= 30` (`sessao_contratada`): `atualiza_nivel_pago` sobe `nivel_pago=1` e `valida_transicao_jornada` recusa etapa abaixo do piso pago (23514 "abaixo do nivel pago"). Em `captado` o passo morre na fixture e parece falha da migration.

**Seed de exemplo:** `scripts/seed-exemplo-completo.ts` (Fase 6) — uma pessoa só (João Pedro Alves Assunção, `origem_dado='exemplo'`), `--etapa <8 etapas>`, `--limpar`, `--sql`; ids determinísticos; cadastra 16 `parametros_metodo` rotulados "EXEMPLO — valor ilustrativo, não é a lei" + `croqui.horas_por_ato`. Os 6 seeds antigos (0016/0026) foram apagados do banco em 05/09.
