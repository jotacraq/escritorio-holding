...
| | |
|---|---|
| **Produção** | `escritorio.grupoparticipa.app.br`, Hostinger Node.js App. ✅ **O deploy É automático por push no remote `infra`** — MEDIDO em 19/09/2026: push de `a7d3155` às 23h5x e, ~90 s depois, a rota que só existe nesse commit (`/api/sessoes/[id]/copiloto/retrospecto`) saiu de **404 para 401**, sem ninguém tocar na Hostinger. 🔴 Isto **corrige** o que esta linha afirmava desde 15/09 ("o deploy não é automático, o remote `infra` não existe mais, exige build manual"): o remote `infra` (`infra-grupo-participa/escritorio-holding`) existe, é ele que publica, e `origin` (`jotacraq/…`) é backup pessoal — em 19/09 o `origin` estava **79 commits atrás** do publicado. **Empurre nos dois.** Como conferir: `/versao.txt` é ÓRFÃO no fluxo git (não é carimbado pelo build), então prove por uma **rota que só exista no commit novo**, nunca por esse arquivo. |
| **Banco** | Supabase `fcfsnqqaphtamhrpuyoh` (sa-east-1), migrations até **0129** aplicadas e provadas (`scripts/verificacao-*.sql`). RLS em toda tabela; links públicos só por RPC (0072). A família do copiloto entra em `public.anonimizar_titular` desde a **0127** (LGPD art. 18) — e a função é `public.`, não `app.`. |
...
| **Fase 7 (fechada 06/09, 3 rodadas)** | "Pronto para apresentar": ligação por IA madura (n8n republicado, pentest ALTO fechado), 0073–0082, vitest 581 testes + CI no GitHub, seed de demo (4 famílias) + `docs/APRESENTACAO.md`, mobile/a11y/consistência; rodada 3: Admin de Formulários/Roteiros (versionado, B15) e Direitos do titular (LGPD: exportar/anonimizar), `api/` por domínio e `dynamic()`. Próximo passo é configuração do João (topo do `CONTINUAR-AQUI.md`). |
