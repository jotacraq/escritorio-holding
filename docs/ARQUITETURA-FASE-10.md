...
| **B71** | 🔴 **REVOGADA POR INTEIRO em 17/09/2026 (decisão do dono/Marcio; ver F7 em `PainelCopiloto.tsx`/`globals.css`).** ~~A sugestão aparece sozinha na tela, ou só quando a advogada pede? Sugestão que aparece durante a fala do cliente rouba a atenção dela na hora errada~~ | 🔴 **REVOGADO.** A cláusula "**nada pisca, nada toca, nada abre sozinho**" deixou de valer como regra geral: o dono pediu "quero que o sistema seja VIVO, tenha animação, notificação visual, cores mudando em novos insights, gatilhos visuais para auxiliar o advogado". Vocabulário único de movimento aprovado em F7 (`entrar-insight`, `decair-destaque`, `preencher-pilula`, `pulsar-uma-vez`, `respirar`, `pensando-pontos`) — geometria constante (nunca altura/posição), `prefers-reduced-motion` zera tudo, sem som, `aria-live` só em `EstadoDoCopiloto`. O que SOBREVIVE da B71 original (não revogado, ver B70): sugestão nova continua exigindo o clique/ciclo do servidor para EXISTIR — nunca um timer client-side gerando conteúdo sozinho; só a ANIMAÇÃO de como ela aparece na tela mudou |
