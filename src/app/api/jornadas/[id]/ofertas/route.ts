...
/**
 * GET /api/jornadas/[id]/ofertas — histórico de ofertas da jornada. Leitura é
 * `eh_interno()` na RLS (`of_sel`, 0011) — mais aberta que a escrita
 * (`admin`/`advogada`, `of_wr`): quem atende o pós-sessão precisa ver o que
 * foi ofertado, mesmo sem poder registrar oferta nova.
 */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ itens: (itens as Oferta[] | null) ?? [] });
  } catch (erro) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const valorOfertadoPadrao =
      corpo.condicao === "incentivo_resolvedor" ? VALOR_INCENTIVO_RESOLVEDOR_CROQUI : VALOR_PADRAO_CROQUI;
    ...
    const { data: oferta, error } = await supabase
      ...
      .insert({
        ...
        produto_id: produtoId,
        valor_padrao: VALOR_PADRAO_CROQUI,
