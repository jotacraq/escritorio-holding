 ...
 */
export function GaleriaGraficos() {
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-6 pb-16">
      <header className="flex flex-col gap-3">
        <h1 className="text-titulo font-bold text-tinta">Galeria de gráficos do Croqui</h1>
        <p className="max-w-2xl text-sm text-tinta-suave">
          Cada peça abaixo é um componente puro de <code className="font-mono text-legenda">src/components/graficos/</code>, SVG inline, sem
          dependência nova. Os controles trocam o <code className="font-mono text-legenda">tema</code> e o{" "}
          <code className="font-mono text-legenda">modoApresentacao</code> recebidos por prop — não há CSS resolvendo isso sozinho (§3.5 do
          plano de arquitetura).
        </p>

        <div className="nao-imprimir flex flex-wrap items-center gap-4 rounded-controle border border-linha bg-papel-elevado px-4 py-3">
          <fieldset className="flex items-center gap-2">
            <legend className="sr-only">Tema do gráfico</legend>
            <span className="text-rotulo font-medium uppercase text-tinta-fraca">Tema</span>
            {(["claro", "escuro"] as const).map((valor) => (
              <label key={valor} className="inline-flex min-h-11 items-center gap-1.5 text-sm text-tinta">
          ...
          <label className="inline-flex min-h-11 items-center gap-1.5 text-sm text-tinta">
      ...
      <div className="flex flex-col gap-6 rounded-controle p-1 transition-colors" style={{ background: fundoPreview }}>
...
function Cartao({ numero, slide, nome, children }: { numero: number | string; slide: string; nome: string; children: React.ReactNode }) {
  return (
    <section className="flex flex-col gap-2">
      <p className="nao-imprimir text-rotulo font-medium uppercase text-tinta-fraca">
