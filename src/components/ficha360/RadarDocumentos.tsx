...
/** Quantas linhas pendentes o radar mostra antes do botão "Ver os N itens". */
const LIMITE_LINHAS_RADAR = 5;
...
function BlocoLado({
  ...
  // Fase 6 — o radar abria com as 33 linhas das duas listas e virava a maior
  // parte da ficha de um cliente que já entregou tudo. O que exige ação é o
  // que FALTA; o que já foi conferido é consulta.
  //
  // Regra: mostra os pendentes (até 5) e esconde os resolvidos atrás de "ver
  // todos". Nada some do DOM por CSS — a lista completa é renderizada quando
  // pedida, e o contador em cima ("15 de 18 prontos") já diz o tamanho real.
  const pendentes = itens.filter((i) => i.estado !== "conferido" && i.estado !== "recebido");
  const visiveis = verTudo ? itens : pendentes.slice(0, LIMITE_LINHAS_RADAR);
  ...
  return (
    <div className="flex flex-col gap-item">
      ...
      {escondidos > 0 && (
        <div>
          <Botao variante="fantasma" tamanho="compacto" onClick={() => setVerTudo((v) => !v)} aria-expanded={verTudo}>
            {verTudo ? "Mostrar só o que falta" : `Ver os ${itens.length} itens`}
          </Botao>
        </div>
      )}
      {itens.length > 0 && visiveis.length === 0 && (
        <p className="text-sm text-tinta-suave">Nada pendente — {prontos} de {total} conferidos.</p>
      )}
    </div>
  );
}
